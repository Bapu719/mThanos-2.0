...
def ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if not user_id:
        message.reply_text("Jisko ban karna h wo mil nhi rha 😛...")
        return ""
    ...
    try:
        ...
        if excp.message == "User not found":
            message.reply_text("Ye kaha gya?😛😛")
            return ""
        else:
            raise

    if is_user_ban_protected(chat, user_id, member):
        message.reply_text("Admins ko ban nhi kar sakta😁...")
        return ""

    if user_id == bot.id:
        message.reply_text("Naa munna mere sath majak nhi😀...")
        return ""
    ...
    try:
        ...
        if excp.message == "Wo msg delete ho gya":
            # Do not reply
            message.reply_text('Banned!', quote=False)
            return log
        else:
            LOGGER.warning(update)
            LOGGER.exception("ERROR %s in chat %s (%s) due to %s", user_id, chat.title, chat.id,
                             excp.message)
            message.reply_text("Pata nhi wo ban nhi ho rha😁..")
...
def temp_ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member):
        message.reply_text("Admins ko ban kar sakta😁....")
        return ""
    ...
    if not reason:
        message.reply_text("Kabtak ban rakhna h wo specify karke fir command karo!")
        return ""
    ...
    try:
        ...
        bot.send_sticker(chat.id, BAN_STICKER)  # banhammer marie sticker
        message.reply_text("Banned! ab ye {} itne time se pehle nhi ayega.".format(time_val))
        return log
...
def kick(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if res:
        ...
        message.reply_text("Unknown ERROR occured calling for (OWNER OF ME)[https://t.me/m_anti_korona].")
...
def kickme(bot: Bot, update: Update):
    ...
    if is_user_admin(update.effective_chat, user_id):
        update.effective_message.reply_text("Babu bhaiya ap admin ho😛😛...")
        return

    res = update.effective_chat.unban_member(user_id)  # unban on current user = kick
    if res:
        update.effective_message.reply_text("Thik babu bhaiya jaisi apki icha😁.")
    else:
        update.effective_message.reply_text("Nhi ho rha😞...")
...
def unban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if is_user_in_chat(chat, user_id):
        message.reply_text("Wo group se ban nhi hua tha aur pehle se hi group me😛..")
        return ""

    chat.unban_member(user_id)
    message.reply_text("Ok!Bulao unko😀..")
...
def rban(bot: Bot, update: Update, args: List[str]):
    ...
    if not args:
        message.reply_text("Kisi chat ki id lagao pata nhi chal rha kidhar kand karna h.")
        return
    ...
    if not user_id:
        ...
        message.reply_text("Kisi chat ki id lagao.")
        return
    ...
    try:
        ...
        if excp.message == "Chat not found":
            message.reply_text("chat mil nhu rha zarur apne galat ID di h ya fir me us group me nhi hu.")
            return
        else:
            raise

    if chat.type == 'private':
        message.reply_text("I'm sorry!Private h")
        return

    if not is_bot_admin(chat, bot.id) or not chat.get_member(bot.id).can_restrict_members:
        message.reply_text("Log restrict nhi ho rhe pehle ye dekho ki me udhar group me admin hu aur permissions h mere pas.")
        return
    ...
    try:
        ...
        if excp.message == "Reply message not found":
            ...
            LOGGER.exception("ERROR banning user %s in chat %s (%s) due to %s", user_id, chat.title, chat.id,
                             excp.message)
            message.reply_text("kuch to seen h 😞.")
...
__help__ = """
 - /kickme: jisne use kiya wo jayega group se

*Admin only:*
 - /ban <userhandle>: bans a user.
 - /tban <userhandle> x(m/h/d): bans a user for x time.. m = minutes, h = hours, d = days.
 - /unban <userhandle>: unbans a user. 
 - /kick <userhandle>: kicks a user,
"""
