...
def warn(user: User, chat: Chat, reason: str, message: Message, warner: User = None) -> str:
    ...
    if num_warns >= limit:
        ...
        if soft_warn:  # kick
            ...
            reply = "{} warns, {} Laat mari G* pe😁!".format(limit, mention_html(user.id, user.first_name))

        else:  # ban
            chat.kick_member(user.id)
            reply = "{} warnings, {} Nikl Law* pehli fursat se BANNED😁!".format(limit, mention_html(user.id, user.first_name))
        ...
        reply = "{} has {}/{} warnings... Tu sudharja beta!".format(mention_html(user.id, user.first_name), num_warns,
                                                             limit)
        if reason:
...
def button(bot: Bot, update: Update) -> str:
    ...
    if match:
        ...
        if res:
            ...
            update.effective_message.edit_text(
                "Tum sare milke pagal bana rhe m* ke bache.".format(mention_html(user.id, user.first_name)),
                parse_mode=ParseMode.HTML)
...
def reset_warns(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id:
        ...
        message.reply_text("Hata di sari warning!")
...
def warns(bot: Bot, update: Update, args: List[str]):
    ...
    if result and result[0] != 0:
        ...
        if reasons:
            text = "Is bane ke pas {}/{} warnings h , Ye sab VAJAH h warns ki:".format(num_warns, limit)
            ...
            update.effective_message.reply_text(
                "User has {}/{} warnings, But koi reason nhu bataye m* ke bache.".format(num_warns, limit))
    else:
        update.effective_message.reply_text("Ye sahi insan h warning nhu h abhi tak ek bhi!")
...
def remove_warn_filter(bot: Bot, update: Update):
    ...
    for filt in chat_filters:
        if filt == to_remove:
            ...
            msg.reply_text("DONE😁 Sab marenge jo bhi ye karega.")
            raise DispatcherHandlerStop

    msg.reply_text("Warning filter nhi h ye abhi ka - run /warnlist for all active warning filters.")
...
def list_warn_filters(bot: Bot, update: Update):
    ...
    if not all_handlers:
        update.effective_message.reply_text("KOI warning filter nhu h idhar LOL😁!")
        return
...
def set_warn_limit(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if args:
        if args[0].isdigit():
            if int(args[0]) < 3:
                msg.reply_text("Minimum warn limite 3 h😁!")
            else:
...
def set_warn_strength(bot: Bot, update: Update, args: List[str]):
    ...
    if args:
        if args[0].lower() in ("on", "yes"):
            ...
            msg.reply_text("Jyada warns pe ban kiya jayega 😁!")
            return "<b>{}:</b>\n" \
                   "<b>Admin:</b> {}\n" \
            ...
            msg.reply_text("Jyada warns pe KICK kiya jayega. VAPS ajayega wo ma* ka baccha")
            return "<b>{}:</b>\n" \
                   "<b>Admin:</b> {}\n" \
