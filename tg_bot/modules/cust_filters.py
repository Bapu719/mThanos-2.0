...
HANDLER_GROUP = 10
BASIC_FILTER_STRING = "*Ye h filters group ke😁:*\n"
...
def filters(bot: Bot, update: Update):
    ...
    if len(extracted) >= 2:
        ...
        if not content:
            msg.reply_text("There is no note message - Sirf khali buttons Telegram allow nhi karega until hum usme kuch text na lagaye")
            return
        ...
        msg.reply_text("Samjaa! Lekin reply kya karna hoga?😀")
        return
...
def stop_filter(bot: Bot, update: Update):
    ...
    for keyword in chat_filters:
        if keyword == args[1]:
            ...
            update.effective_message.reply_text("Filter *{}* band ho gya.".format(chat_name), parse_mode=telegram.ParseMode.MARKDOWN)
            raise DispatcherHandlerStop

    update.effective_message.reply_text("Ye filter list me nhi h😛 - run /filters for all active filters.")
                ...
                try:
                    ...
                    if excp.message == "Unsupported url protocol":
                        message.reply_text("You seem to be trying to use an unsupported url protocol. Telegram "
                                           ...
                                           "again, or ask in @telegram for help.")
                        ...
                        message.reply_text("This note could not be sent, as it is incorrectly formatted. Ask in "
                                           "@Telegram if you can't figure out why!")
